...
struct Gain
{
    ...
    void incrementGain()
    {
        if (derampGain) { gain = (gain <= CASPI::Constants::zero<FloatType>) ? CASPI::Constants::zero<FloatType> : gain - gainIncrement; }
        else if (rampGain) { gain = std::min(gain + gainIncrement,CASPI::Constants::one<FloatType>); }

        if (rampGain & gain >= CASPI::Constants::one<FloatType>) { rampGain = false; }
        else if (derampGain & gain <= CASPI::Constants::zero<FloatType>) { derampGain = false; }
    }
