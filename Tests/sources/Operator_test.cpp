...
namespace TestHelpers
{
    ...
    double calculateDCOffset(const std::vector<double>& samples)
    {
        return std::accumulate(samples.begin(), samples.end(), 0.0) / samples.size();
    }

    int countZeroCrossings(const std::vector<double>& samples)
    {
        int count = 0;
    ...
    double calculatePeriodCorrelation(const std::vector<double>& samples, size_t period)
    {
    ...
    std::vector<double> applyHannWindow(const std::vector<double>& samples)
    {
        ...
        {
            double w = 0.5 * (1.0 - std::cos(CASPI::Constants::TWO_PI<double> * i / (samples.size() - 1)));
            windowed[i] = samples[i] * w;
        }
        return windowed;
    }

    CArray realToComplex(const std::vector<double>& samples)
    {
    ...
    std::vector<double> getMagnitudeSpectrum(const CArray& fftData)
    {
    ...
    double findPeakFrequency(const std::vector<double>& spectrum, double sampleRate, size_t fftSize)
    {
        auto it = std::max_element(spectrum.begin(), spectrum.end());
        size_t idx = std::distance(spectrum.begin(), it);
        return idx * sampleRate / fftSize;
    }

    int countSignificantPeaks(const std::vector<double>& spectrum, double threshold = 0.1)
    {
