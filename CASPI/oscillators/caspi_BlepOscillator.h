    ...
    namespace Oscillators
    {
        ...
        namespace detail
        {
            ...
            CASPI_ALWAYS_INLINE FloatType polyBlep (FloatType phase, FloatType dt) noexcept
            {
                ...
                return (phase < dt) ? blepRise : (phase > FloatType (1) - dt) ? blepFall : FloatType (0);
            }
            ...
            : public Graph::AudioNode<BlepOscillator<FloatType>, FloatType>
        {
            ...
            public:
                ...
                // ---- Graph hooks (CRTP, not virtual) ----

                /**
                 * Called by AudioNode::prepareToRender() after sample rate is stored
                 * and outputBuffer is resized and cleared.
                 */
                void onPrepare (std::size_t /*numChannels*/, std::size_t /*numFrames*/, double sampleRateIn) noexcept
                {
                ...
                {
                    Graph::NodeBase<FloatType>::setSampleRate (newRate);
                    const FloatType hz = frequency.value();
                 ...
                 * @endcode
                 */
                void renderBlock (FloatType* CASPI_RESTRICT output, int numSamples) noexcept CASPI_NON_BLOCKING
                {
            ...
            private:
                 ...
                 */
                CASPI_NO_DISCARD FloatType computeSample (FloatType p, FloatType dt, FloatType correction) noexcept
                    CASPI_NON_BLOCKING
                {
                    switch (shape)
                    {
                        ...
                        case WaveShape::Pulse:
                        {
                            ...
                            return naive - detail::polyBlep (p, dt) + detail::polyBlep (p2, dt) + correction;
                        }
                        case WaveShape::Triangle:
                        {
                            const FloatType naive = (p < FloatType (0.5)) ? FloatType (-1) : FloatType (1);
                            const FloatType p2    = std::fmod (p + FloatType (0.5), FloatType (1));
                            const FloatType sq    = naive - detail::polyBlep (p, dt) + detail::polyBlep (p2, dt);
                            triangleIntegrator    = triangleIntegrator * triangleLeak + FloatType (4) * dt * sq;
                ...
                void initParameters() CASPI_ALLOCATING
                {
                    ...
                    frequency.setRange (FloatType (20), FloatType (20000), Core::ParameterScale::Logarithmic);
