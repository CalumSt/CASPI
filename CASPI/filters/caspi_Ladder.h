 ...
 * @author CS Islay
 * @brief  Stub: Filter<FloatType, FilterTopology::Ladder>
 *
 * Placeholder for the Moog-style four-pole ladder filter.
 * Currently acts as an identity pass-through until implemented.
 *
 * STATE LAYOUT (NumStates = 4)
 *
 *   states[0..3]  z^-1 delay registers for each ladder stage
 *
 * COEFFICIENT LAYOUT (NumCoeffs = 6)
 *
...
namespace CASPI
{
    namespace Filters
    {
        ...
        template <>
        struct FilterTraits<FilterTopology::Ladder>
        {
            static constexpr std::size_t NumStates = 4;
            static constexpr std::size_t NumCoeffs = 6;
        };

        template <CASPI_FLOAT_TYPE FloatType>
        class Filter<FloatType, FilterTopology::Ladder>
            : public FilterBase<Filter<FloatType, FilterTopology::Ladder>,
                                FloatType,
                                FilterTraits<FilterTopology::Ladder>::NumStates,
                                FilterTraits<FilterTopology::Ladder>::NumCoeffs>
        {
            public:
                using Base = FilterBase<Filter<FloatType, FilterTopology::Ladder>,
                                        FloatType,
                                        FilterTraits<FilterTopology::Ladder>::NumStates,
                                        FilterTraits<FilterTopology::Ladder>::NumCoeffs>;
                ...
                /**
                 * @brief Compute or recompute all ladder coefficients from the
